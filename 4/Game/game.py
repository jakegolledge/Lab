...
def remove_punct(text):
    ...
    'goSouTh'
    """
    #steps through each character within text and appends to new text if it is not punctuation
...
def normalise_input(user_input):
    ...
    'help'
    """
    user_input = "".join(i for i in user_input if i not in string.punctuation)
...
def exit_leads_to(exits, direction):
    ...
    #leads = rooms[next_location]
    #return leads
    for i in exits:
        #move_exit = exits[i]
        s = "".join(x for x in exits[direction])
        return rooms[s]["name"]
...
def print_menu(exits):
    ...
    for key in exits:
        leads = str(exit_leads_to(exits, key))
        print_menu_line(key, leads)
...
def menu(exits):
    ...
    while True:
        ...
        if normalise_input(where) in exits:
            return normalise_input(where)
        else:
...
def move(exits, direction):
    ...
    #for i in exits:
    #    move_exit = exits[i]
    #    s = "".join(x for x in move_exit)
    #    if rooms[s] == direction:
    #        return rooms[s]
    #move_room = rooms[s] 
    #return rooms[s]
    #move_location = move_room[direction]
    #if move_room == rooms[direction]:
