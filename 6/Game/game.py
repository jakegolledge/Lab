...
def execute_go(direction):
    ...
    """
    #update current room to be the room that the direction was in
    #from player import current_room
    global current_room
    exits = current_room["exits"]
    #print(direction)
    #for i in direction:
    #    s = "".join(x for x in direction)
    if direction in exits:
        current_room = rooms[current_room["exits"][direction]]
        #return current_room
        #return current_room[exit_leads_to(exits, direction)]
        #room = current_room["exits"][direction]
    else:
        print("You cannot go there.")
        
#def create_item_list(items):
#	"""This function takes items as a """
#	items_string = ""
#   for i in items:
#        items_string += ", " + str(i["name"])
#    items_string = items_string[2:]
...
def execute_take(item_id):
    ...
    print("You cannot take that.")
    		#break
    #for i in item_id:
    #	item_ref = "".join(x for x in item_id)
    #item_ref = "item_" + item_ref
    #print(current_room["items"])
    #location = current_room["items"].index(item_ref)
    #print(location)
    """if ITEM IS IN ROOM:
    	inventory.append(DICTIONARY FOR ITEM)
    	print(inventory)
    	print(item_id)
    else:
    	print("You cannot take that.")
...
def execute_command(command):
    ...
    if command[0] == "go":
        if len(command) > 1:
            ...
            #current_room = execute_go(command[1])
            #return current_room
        else:
            print("Go where?")
